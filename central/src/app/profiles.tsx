...
export type MemberRole = 'admin' | 'lideranca' | 'comercial' | 'crm' | 'social' | 'design' | 'infra'
...
interface ProfilesCtx {
  ...
  /** Altera e-mail e/ou senha de um usuário (via função serverless — precisa da service_role). */
  updateUserCredentials: (id: string, patch: { email?: string; password?: string }) => Promise<{ error: string | null }>
...
export function ProfilesProvider({ children }: { children: React.ReactNode }) {
  ...
  /** Altera e-mail e/ou senha via função serverless (precisa da service_role no servidor). */
  const updateUserCredentials = useCallback(
    async (id: string, patch: { email?: string; password?: string }) => {
      const { data } = await supabase.auth.getSession()
      const token = data.session?.access_token ?? ''
      try {
        ...
        const body = await res.json().catch(() => ({}))
        if (!res.ok) return { error: body.error ?? `Falha ao atualizar usuário (${res.status})` }
        await refresh()
        return { error: null }
      } catch {
        return { error: 'Não foi possível contatar o servidor (só funciona em produção).' }
      }
    },
    [refresh],
  )
  ...
  const value = useMemo<ProfilesCtx>(
    () => ({
      members, loading, getMember, refresh, updateMember, setMemberAvatar,
      createUser, updateUserCredentials, removeUser,
    }),
    [members, loading, getMember, refresh, updateMember, setMemberAvatar, createUser, updateUserCredentials, removeUser],
  )
