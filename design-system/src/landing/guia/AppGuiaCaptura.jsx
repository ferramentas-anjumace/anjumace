...
/* Página de captura do guia "Os cinco tipos de falha" (/guia).
   ...
   18→20, headlines 48px no desktop), cards com selo cheio + sombra +
   elevação no hover, filete conectando os 5 tipos de falha no mobile
   (mesmo padrão do StepTile de /guia/obrigado), Reveal alternando
   esquerda/direita nas headlines. */
...
/** Mini-card com traço sálvia — os bullets do problema.
    ...
    Elevação no hover — mesma linguagem premium do resto do funil (24/07). */
function MiniCard({ title, children, tone = 'dark', image, imageAlt = '', imageLabel }) {
  const light = tone === 'light'
  return (
    <div className={`flex flex-col gap-3 rounded-2xl border p-6 shadow-sm transition-shadow duration-moderate hover:shadow-md ${light ? 'border-graphite-900/10 bg-cream-50' : 'border-cream-100/10 bg-cream-100/5'}`}>
      {image ? (
        <img
          src={image}
          alt={imageAlt}
      ...
      <p className={`text-[16px] leading-relaxed tracking-tight lg:text-[18px] ${light ? 'text-graphite-900/75' : 'text-cream-100/75'}`}>
...
/** Card de falha com tile de numeral romano — eco da capa do e-book.
    As cores intercalam sálvia/dourado entre os boxes. Selo cheio com
    gradiente + sombra (em vez de contorno vazado) + trilha vertical
    conectando um numeral ao próximo — sempre 1 coluna aqui, então a trilha
    fica em qualquer tela (mesmo padrão do StepTile de /guia/obrigado,
    aplicado 24/07 pra alinhar as 4 páginas do funil). */
function FalhaCard({ roman, title, children, accent = 'sage', isLast = false }) {
  const gold = accent === 'gold'
  return (
    <div className={`relative flex gap-5 rounded-2xl border p-6 shadow-sm transition-shadow duration-moderate hover:shadow-md ${gold ? 'border-gold-300/25 bg-gold-300/5' : 'border-sage-500/25 bg-sage-500/10'}`}>
      {!isLast && (
        <span aria-hidden className="absolute -bottom-4 left-12 h-4 w-px bg-graphite-900/15" />
      )}
...
/** Callout com filete dourado — a frase que fica. Selo com ícone (Lightbulb)
    + brilho leve passando por cima — mesmo padrão premium de /guia/obrigado
    (24/07). */
  ...
  return (
    <main className="page-largura-1050 bg-[#FAFFF2] text-graphite-900">
      ...
      <section id="inicio" className="relative flex min-h-dvh scroll-mt-16 flex-col overflow-hidden">
        <Nav links={NAV_LINKS} ctaLabel="Quero receber o guia" ctaHref="#formulario" />
      ...
      {/* ---------------------------------------------------- 3 · A SOLUÇÃO */}
      <section id="solucao" className="scroll-mt-16 border-t border-graphite-900/10 bg-[#F5F5DD] py-20 md:py-28">
        <div className="container flex max-w-3xl flex-col gap-10">
          <Reveal variant="right" className="flex flex-col gap-4">
            <h2 className="text-h2 text-graphite-900 md:text-[48px]">Cinco limites. Cinco decisões técnicas diferentes.</h2>
          ...
          <div className="flex flex-col gap-4">
            <Reveal variant="scale" delay={0}><FalhaCard roman="I" title="Falha técnica.">O único sinal que manda encerrar imediatamente, mesmo com a ficha pela metade. Não porque o músculo acabou, mas porque ele deixou de ser o protagonista da série.</FalhaCard></Reveal>
            <Reveal variant="scale" delay={80}><FalhaCard roman="II" accent="gold" title="Falha concêntrica.">O limite que vale procurar, com a lista dos exercícios onde procurá-lo é seguro e a dos exercícios onde procurá-lo é erro. É aqui que moram as suas três repetições.</FalhaCard></Reveal>
            <Reveal variant="scale" delay={160}><FalhaCard roman="III" title="Falha excêntrica.">A camada de capacidade que ainda existe depois do "não consigo mais", e o critério que decide quando ela pode ser aberta. Poucas mulheres sabem que ela existe. Menos ainda sabem quando usá-la.</FalhaCard></Reveal>
            <Reveal variant="scale" delay={240}><FalhaCard roman="IV" accent="gold" title="Falha neural.">Por que a coordenação some antes de a perna acabar, e por que isso muda a ordem inteira dos exercícios da sua sessão.</FalhaCard></Reveal>
            <Reveal variant="scale" delay={320}><FalhaCard roman="V" title="E a quinta," isLast>a mais comum de todas, que não acontece no músculo nem no comando. É ela que explica as três repetições que você entrega de graça, série após série, sem nunca perceber.</FalhaCard></Reveal>
          </div>
      ...
      <section id="formulario" className="scroll-mt-16 border-t border-graphite-900/10 bg-[#FAFFF2] py-20 md:py-28">
        <div className="container max-w-xl">
          ...
          <form
            ref={formRef}
            onSubmit={submit}
            className="flex flex-col gap-5 rounded-3xl border border-graphite-900/10 bg-cream-50 p-8 shadow-sm md:p-10"
          >
            <div className="flex flex-col gap-2">
              <label htmlFor="guia-nome" className="whitespace-nowrap text-[10px] uppercase tracking-wide text-graphite-900/60 sm:text-caption sm:tracking-wider">Nome</label>
              <input
                id="guia-nome"
                type="text"
                value={name}
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="guia-email" className="whitespace-nowrap text-[10px] uppercase tracking-wide text-graphite-900/60 sm:text-caption sm:tracking-wider">O melhor e-mail para receber o guia</label>
              <input
                id="guia-email"
                type="email"
                value={email}
      ...
      <footer className="border-t border-graphite-900/10 bg-[#F5F5DD] py-10">
        <div className="container flex justify-center">
          <img src="/logo-anju-dark.svg" alt="Anju Mace" className="h-3.5 w-auto opacity-50" />
        </div>
      </footer>
    </main>
  )
