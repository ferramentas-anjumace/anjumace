        ...
        {CURSOS.items.map((curso, i) => (
          ...
          >
            {curso.cover ? (
              <img
                src={curso.cover}
                alt={`Capa do curso ${curso.name}`}
                className="mb-2 aspect-video w-full rounded-2xl object-cover"
              />
            ) : (
              <ImagePlaceholder
                tone="dark"
            ...
            {/* Headline em dourado: é o que diferencia um curso do outro à
                primeira vista — o corpo fica neutro embaixo. */}
            <h3 className="font-display text-2xl font-light leading-snug text-accent-2">
              {curso.headline}
            </h3>
            <p className="text-body-sm text-content-inverse/70 leading-relaxed">{curso.text}</p>

            {/* Módulos como chips — escaneável, sem virar mais um bloco de texto. */}
            <ul className="mt-1 flex flex-wrap gap-2 border-t border-white/10 pt-5">
              {curso.modules.map((m) => (
                <li
                  key={m}
                  className="rounded-full border border-accent-2/25 bg-accent-2/10 px-3.5 py-1.5 text-body-sm text-content-inverse/85"
                >
                  {m}
                </li>
              ))}
